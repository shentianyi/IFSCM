...
function get_org_printers() {
     $.post('../organisation_manager/get_printer', {
          org : $("#org_id").val(),
          type : 100,
          ptype : $("#org_printerType").val()
     }, function(data) {
          $("#org_printer_result").html(data);
     });
}

function add_org_printer() {
     if($("#org_template").val().length > 0 && $("#org_moduleName").val().length > 0) {
          $.post('../organisation_manager/add_printer', {
               org : $("#org_id").val(),
               type : 100,
               ptype : $("#org_printerType").val(),
               template : $("#org_template").val(),
               moduleName : $("#org_moduleName").val()
          }, function(data) {
               alert(data.msg);
          });
     } else {
...
function del_org_printer() {
     $.post('../organisation_manager/del_printer', {
          type:100,
          printerKey : $("#org_printerKey").val()
     }, function(data) {
          alert(data.msg);
     });
}

function add_org_defalut_printer() {
     $.post('../organisation_manager/add_default_printer', {
          type:100,
          printerKey : $("#org_printerKey").val()
     }, function(data) {
...
function update_org_default_printer() {
     if($("#org_printerKey").val().length > 0) {
          $.post('../organisation_manager/update_default_printer', {
               printerKey : $("#org_printerKey").val(),
               type:100,
               updated : $("#org_updatedcheck").attr('checked') == "checked",
               template : $("#org_template").val(),
               moduleName : $("#org_moduleName").val()
          }, function(data) {
               alert(data.msg);
